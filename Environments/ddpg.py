...
class ReplayBuffer():
	...
	def sample(self, n):
		...
		return torch.stack(s_lst, dim=0), torch.stack(a_lst, dim=0), \
			   torch.tensor(r_lst, dtype=torch.float32), torch.stack(s_prime_lst, dim=0), \
...
def main():
	...
	for n_epi in range(10000):
		...
		for t in range(100): # maximum length of episode is 200 for Pendulum-v0
			a = mu(s).detach().to(torch.float32) 
			a = a + torch.from_numpy(ou_noise()).to(dtype=torch.float32)
